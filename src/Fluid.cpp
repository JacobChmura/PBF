...
void Fluid::step(Eigen::MatrixXd &fluid_state, Eigen::MatrixXd &colors, Eigen::Vector3d mouse_pos, bool add_user_force, bool use_viscocity, bool use_vorticity){
	...
	// ----------- Apply External forces ----------
        ...
        if (add_user_force) {
                user_force_direction= (-fluid_state).rowwise() + mouse_pos.transpose();
                user_force_direction.rowwise().normalize();
                v += particle_mass * user_f(0) * user_force_direction;
        }
	...
	//Update Velocity
        ...
        auto t7 = Clock::now();
        if (DEBUG) std::cout << "Vorticity and Viscocity [" << std::chrono::duration_cast<std::chrono::milliseconds>(t7 - t6).count() << " ms]\n";


        // ----------- Update Global State and Spatial Hash Grid ---------
        fluid_state = x_new;
        grid.update(fluid_state);
        ...
        auto t8 = Clock::now();
        if (DEBUG) std::cout << "Simulation Step Total Time [" << std::chrono::duration_cast<std::chrono::milliseconds>(t8 - t0).count() 
                << " ms]\n----------------------------------------\n";
